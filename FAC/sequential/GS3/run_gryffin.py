...
OBS_FILE = "data.csv"
sampling_strategies = np.array([0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 0.99])
#sampling_strategies = np.array([i*2/8-1  for i in range(8)]).astype(float)
...
CONFIG = {
    ...
    "parameters": [
          {"name": "seq", "type": "categorical", "category_details": {"a": None, "b": None, "c": None, "d": None, "e": None, "f": None}},
          {"name": "t0", "type": "discrete", "low": 1, "high": 22},
          {"name": "t1", "type": "discrete", "low": 1, "high": 22}
    ],
    "objectives": [
          {"name": "cv", "goal": "min", "tolerance":0.5, "absolute": True}
    ]
}
...
df_out.to_csv(OBS_FILE, index=False)
